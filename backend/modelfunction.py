...
#import csv files
file_path = 'Datasets/symtoms_df.csv'
symptom = pd.read_csv(file_path)
precuation=pd.read_csv('Datasets/precautions_df.csv')
...
randomforest=pickle.load(open('models/randomforest.pkl','rb'))

#==================================================================================#

def predict_disease(symp):
    input_array=np.zeros(len(symptoms_dict))
    
    for item in symp:
        input_array[symptoms_dict[item]]=1
        predict_d=randomforest.predict([input_array])[0]
    return predict_d
...
#     prec=precuation[precuation['Disease']==dis][['Precaution_1','Precaution_2','Precaution_3','Precaution_4']];
...
def mainfunction(symp):
    disease=predict_disease(symp)
    response=helper(disease)
    return response